...
- **設定ファイルの動作確認**
  - `config.yaml` を自身の Discord Bot / Channel ID / URL / targets にあわせて書き換える。
  - 以下を実行し、設定ロードだけが成功するか確認する。
  - 例: `python -m us_amex_offer_hunter.cli.main --dry-run-settings` のような専用フラグを今後追加予定（現時点は `pytest tests/test_settings.py` で代替）。
...
- **Discord 通知の検証（トークンを設定できる場合）**
  - `config.yaml` に実際の Discord Bot Token / Channel ID を設定。
  - `pytest tests/test_notifier_discord.py -q` で基本的なインターフェースが壊れていないかを確認（ネットワーク I/O 自体はモック前提）。
  - 将来的には、専用コマンド（例: `us-amex-offer-hunter notify-test`）で 1 通だけテスト通知を送る機能を追加予定。
...
- `SeleniumEngine` でヘッドレス Chrome を起動し、`config.yaml` の `urls` を順番に巡回。
...
- `SeleniumEngine` に以下のパラメータを導入（`Settings` / `config.yaml` 連携）:
  - ページロードタイムアウト
  - 要素検出タイムアウト
  - 最大リトライ回数
...
- プロキシ利用有無を設定 (`config.yaml`) から制御可能にする。
